...
class AuthController extends CI_Controller
{
    ...
    public function teacherRegistrationStore()
    {
        ...
        $this->email->subject("Thank you for signing up with BrighShine");
        $this->email->message(
            $this->load->view("web/email_template", $emailData, true)
        );
        $this->email->send();
        if (!$userCreation) {
            $this->session->set_flashdata(
                "error",
                "Something is wrong! You are not registered"
            );
            return redirect(base_url("/teacher-registration"));
        } else {
            $this->session->set_flashdata(
                "success",
                "Select your books to register with BrightShine and wait for verification. Check your registered email id for your account details."
            );

            $res = $this->AuthModel->validate_web(
    ...
    public function studentRegistrationStore()
    {
        ...
        if (empty($checkTeacherCode)) {
            $this->session->set_flashdata(
                "error",
                "Teacher Code is Not Valid!!"
            );
            return redirect(base_url("student-registration"));
        } elseif ($checkTeacherCode === "limit_exhausted") {
            $this->session->set_flashdata(
                "error",
                "Teacher Code has reached its limit!!"
            );
            return redirect(base_url("student-registration"));
        } else {
            $data = [
            ...
            if ($userCreation) {
                $this->APIModel->assignBooksToStudent(
                    $userCreation->id,
                    $teacherCode,
                    $classId
                );
                ...
                $this->email->to($this->input->post("email"));
                $this->email->from(
                    "info@brightshinedigital.in",
                    "Brightshine Digital"
                );
                $this->email->cc(
                    "mayank@epochstudio.net, info@brightshinedigital.in"
                );
                $this->email->subject(
                    "Thank you for signing up with BrighShine"
                );
                $this->email->message(
                    $this->load->view("web/email_template", $emailData, true)
                );
                ...
                $this->session->set_flashdata(
                    "success",
                    "You are registered with BrightShine, please wait for verification. Check your registered email id for your account details."
                );
                return redirect(base_url("student-registration"));
            } else {
                $this->session->set_flashdata(
